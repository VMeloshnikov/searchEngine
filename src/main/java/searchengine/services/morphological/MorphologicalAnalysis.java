package searchengine.services.morphological;
...
public class MorphologicalAnalysis {
    private static final Log log = LogFactory.getLog(MorphologicalAnalysis.class);
    private static Set<String> parts = null;
    ...
    public TreeMap<String, Integer> textAnalysis (String text) {
    ...
    public ArrayList<Integer> findLemmaIndexInText(String text, String lemma) {
        ArrayList<Integer> listOfIndexed = new ArrayList<>();
        ...
        for(String s1: list) {
            ...
            for (String s2: lemmas) {
                if (s2.equals(lemma)){
                    listOfIndexed.add(index);
                }
            }
            index += s1.length() + 1;
        }
        return listOfIndexed;
    }
    ...
    private boolean isServiceParts (String word) {
        ...
        try {
            ...
            log.debug("Ошибка морфологического анализа. Символы не могут быть проанализированы: "
            + word);
        }
        return is;
    }
